...
/**
    Take image data in yuv420p format and reduce the image dimensions by half.
...
*/
void halfscale_yuv420p( int w, int h, uint8_t* srcBuffer, uint8_t* dstBuffer )
{
}
...
void halfscale_yuyv422( int w, int h, uint8_t* srcBuffer, uint8_t* dstBuffer )
{
    ...
    while ( --r )
    {
        ...
        //__builtin_prefetch( srcBuffer, 0, 0 );
        //__builtin_prefetch( srcBuffer+rowBytes, 0, 0 );
        //__builtin_prefetch( dstBuffer, 1, 0 );
    }
}
